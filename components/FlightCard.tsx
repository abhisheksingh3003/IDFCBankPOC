    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-[#1a1a2e]">{flight.airline}</h4>
            <p className="text-sm text-slate-500">{flight.id}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-red-650">INR {flight.price}</p>
      ...
      <button className="w-full mt-4 bg-[#9D1D27] hover:bg-[#851820] text-white py-2 rounded-lg font-semibold transition-colors">
        Select Flight
      </button>
    </motion.div>
  );
