...
import { Curation, PreTripChecklistItem } from '../types';

interface PreTripConciergeProps {
  curation: Curation;
}

const PreTripConcierge: React.FC<PreTripConciergeProps> = ({ curation }) => {
  ...
  const [items, setItems] = useState<PreTripChecklistItem[]>([
    { id: '1', task: 'Travel Insurance', description: 'Comprehensive medical and trip cancellation coverage.', isCompleted: curation.essentialsBooking?.items.some(i => i.category === 'Protection') || false, category: 'Services', recommendation: 'Mastercard Platinum Shield' },
    ...
    { id: '6', task: 'Set Travel Notifications', description: 'Inform Mastercard of your travel dates to ensure uninterrupted card usage.', isCompleted: true, category: 'Services' }
  ]);
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Perks & Upsells Section */}
        <div className="space-y-8">
          <div className="space-y-6">
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] flex items-center gap-2">
              <ShieldCheck size={16} className="text-red-600" /> Mastercard Perks
            </h3>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl group overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-slate-500 font-medium leading-relaxed mb-6">Mastercard customers get 20% cashback on Global Travel Protection.</p>
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                  <span className="text-xl font-black text-slate-900 dark:text-white">AED 89<span className="text-[10px] text-slate-400 font-bold ml-1">/ person</span></span>
                  <button className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">Protect</button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl group overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-slate-500 font-medium leading-relaxed mb-6">Unlimited 5G data across EU. Instant activation from Mastercard app.</p>
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                  <span className="text-xl font-black text-slate-900 dark:text-white">AED 45<span className="text-[10px] text-slate-400 font-bold ml-1">/ 10GB</span></span>
                  <button className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">Activate</button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl group overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-slate-500 font-medium leading-relaxed mb-6">VIP pickup in {curation.destination.name}. 15% discount for Mastercard holders.</p>
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                  <span className="text-xl font-black text-slate-900 dark:text-white">AED 110<span className="text-[10px] text-slate-400 font-bold ml-1">/ Sedan</span></span>
                  <button className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">Book</button>
                </div>
              </div>
            </div>
          </div>
